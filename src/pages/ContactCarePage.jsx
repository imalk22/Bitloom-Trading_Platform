...
export default function ContactCarePage() {
  ...
  const startChat = () => openSupportChat();
    ...
    <div className="min-h-screen bg-[#0b0f17] text-white flex flex-col items-center justify-center p-4"
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.1, type: "spring", stiffness: 200 }}
          ...
          <div className="relative">
            <div className="h-20 w-20 rounded-full bg-sky-500/15 border border-sky-500/30 flex items-center justify-center shadow-2xl shadow-sky-500/20">
              <Headphones className="h-9 w-9 text-sky-400" />
            </div>
        ...
        <div className="text-center mb-8">
          <h1 className="text-2xl font-black text-white">Bitloom Support</h1>
          <p className="text-slate-500 text-sm mt-2 max-w-sm mx-auto">
            Chat with a support agent for deposits, withdrawals, and account help. Nothing is auto-processed without agent confirmation.
          </p>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
          ...
          <button onClick={() => navigate("/")}
            className="w-full py-3 rounded-2xl bg-slate-900 border border-slate-800 text-slate-400 font-semibold text-sm hover:border-slate-700 hover:text-slate-300 transition cursor-pointer flex items-center justify-center gap-2">
